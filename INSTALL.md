...
Приложение использует встроенную систему аутентификации:
- API ключ вводится при первом запуске
- После проверки ключа генерируется 4-значный PIN
- Для последующих входов используется PIN код
- Возможность сбросить ключ и ввести новый

### Поддерживаемые ключи:
1. OpenRouter (начинаются с `sk-or-v1-...`)
2. VSEGPT (начинаются с `sk-or-vv-...`)
...
3. При первом запуске:
   ...
   - Дождитесь проверки ключа
   - Запомните сгенерированный PIN код

4. При последующих запусках:
   - Вводите только PIN код
   - Для смены ключа используйте "Сбросить ключ"
